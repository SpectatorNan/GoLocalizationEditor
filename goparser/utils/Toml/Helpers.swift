...
extension Toml {
     ...
     */
    static func checkAndSetArray<T: SetValueProtocol>(check: [Any], key: [String], out: inout T) throws {
        // allow empty arrays
        if check.isEmpty {
     ...
     */
    static func trimStringIdentifier(_ input: String, _ quote: String = "\"") -> String {
    ...
    static func getKeyPathFromTable(tokens: [Token]) -> [String] {
    ...
    static func consumeTableIdentifierTokens(tableTokens: inout [Token], tokens: inout [Token]) {
    ...
    static func getTableTokens(keyPath: [String], tokens: inout [Token]) -> [Token] {
    ...
    static func extractTableTokens(tokens: inout [Token], inline: Bool = false) -> [Token] {
        var tableTokens = [Token]()
        while !tokens.isEmpty {
            let tableToken = tokens[0]
