...
public class Toml: CustomStringConvertible, SetValueProtocol {
    ...
    public func table(from path: [String]) -> Toml {
        ...
        for keyName in keyNames {
            ...
            if keyName.begins(with: path) {
                ...
                constructedTable.set(value: self.fetch(keyName.components)!, for: keyArray)
            }
        }
    ...
    */
    public func fetch<T>(_ path: [String]) -> T? {
        if let val = data[Path(path)] {
            return val as? T
        }
    ...
    /**
        ...
        - Returns: value of key path
    */
    public func fetch<T>(_ path: String...) throws -> T? {
        return fetch(path)
    }
    ...
    */
     ...
     */
    public func removeTable(from path: [String]) {
        for keyName in keyNames {
            if keyName.begins(with: path) {
                keyNames.remove(keyName)
                data.removeValue(forKey: keyName)
            }
        }
        for tableName in tableNames {
            if tableName.begins(with: path) {
    ...
    /**
        Update a table in the document with a new table
        
        - Parameter path: Key path of table to update
        - Parameter toml: New table to update with
     
     */
    public func updateTable(with path: [String], toml: Toml) {
        var fulltablePrefix = Path(path)
        removeTable(from: path)
        setTable(key: path)
        for keyName in toml.keyNames { 
            let val = toml.fetch(keyName.components) as String?
            if let val = val {
              let key = fulltablePrefix + keyName
                set(value: toml.fetch(keyName.components)!, for: key.components)
            }
        }
    }
}
